...
def bootstrap():
    ...
    decision_texts = [
        ...
        if (PATTERNS / f"{cat}-DECISION.md").exists()
    ]
    costs = L.cost_map(decision_texts)
    meta = {}
...
FIELD_GLOSSARY = {
    ...
    "cost": "Relative inference cost: baseline|low|medium|medium-high|high|very-high|highest (where the source decision guides tabulate it).",
}
...
`siblings`, `mechanism_refs`, `cost`) are suggestions. See `ingest.json` → `field_glossary`
...
## Inline math
Some content carries inline LaTeX math (e.g. `$\\to$`, `$\\times$`) verbatim from the
canonical source — render with a math-aware viewer or strip on ingest.
