...
## Description
When sub-tasks of a request are genuinely independent of each other, run them simultaneously across distinct LLM calls and aggregate the results programmatically, so wall-clock latency collapses from the sum of the calls to the maximum. This is a condensed digest; the canonical file (`patterns/O4-Parallelization.md`) carries the full decision criteria, failure modes, and implementation.
