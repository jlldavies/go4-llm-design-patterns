...
## Description
Ground a model's response in a specific, external, updatable corpus by retrieving the passages relevant to each query at query time and injecting them into the prompt — without retraining the model. This is a condensed digest; the canonical file (`patterns/K1-Vanilla-RAG.md`) carries the full decision criteria, failure modes, and implementation.
