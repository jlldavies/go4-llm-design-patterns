...
## Description
Let the agent use the existing CLI ecosystem as its tool surface — invoking `git`, `docker`, `gh`, `kubectl`, cloud CLIs, and Unix utilities directly — so the integration carries zero schema-token overhead and inherits decades of battle-tested behaviour. This is a condensed digest; the canonical file (`patterns/I4-CLI-Invocation.md`) carries the full decision criteria, failure modes, and implementation.
