...
## Description
Demonstrate the task with examples in the prompt, so the model learns the desired format and behaviour from the demonstrations themselves rather than from a description of them. This is a condensed digest; the canonical file (`patterns/S2-Few-Shot.md`) carries the full decision criteria, failure modes, and implementation.
