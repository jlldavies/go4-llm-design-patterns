...
## Description
Build a single agent that is competent across all phases of a complex task by stacking the loop primitives each phase needs — exploration on a ReAct loop, planning on plan-execute, repair on generate-test-repair, recovery on bounded retry — rather than trying to force every phase through one primitive. Composes with O17, O4, V15. This is a condensed digest; the canonical file (`patterns/O16-Hybrid-Control-Flow.md`) carries the full decision criteria, failure modes, and implementation.
...
Related: [[O17-Agent-Isolation]] · [[O4-Parallelization]] · [[V15-LLM-as-Judge]] · [[O8-Loop-Agent]] · [[O6-Orchestrator-Workers]] · [[O2-Prompt-Chaining]]
