...
## Description
Promote R7 Reflexion's ephemeral, within-task verbal critiques into a durable lesson library that survives session resets, is injected into future contexts as light-weight guidance, and accumulates compounding improvement over time — giving the agent the closest thing to *learning* available without fine-tuning. Composes with V1. This is a condensed digest; the canonical file (`patterns/H2-Episodic-Self-Improvement.md`) carries the full decision criteria, failure modes, and implementation.
