...
## Description
Give an agent continuity beyond a single context by writing knowledge to an external store and retrieving it when relevant — so the agent improves over time without retraining. This is a condensed digest; the canonical file (`patterns/K10-Long-Term-Memory.md`) carries the full decision criteria, failure modes, and implementation.
